            ...
            <div className=''>
                <nav className=' md:flex md:justify-between md:items-center px-5 text-neutral-200 text-xl font-red-rose'>
                    <div className='flex justify-between items-center'>
                        <span>
                            <Link to='/'><div className='pt-5 pl-5 md:p-0'><img className='cursor-pointer w-10 h-10' src={imgIcon} alt=''/></div></Link> {/* Create Vindiversity Icon in Canva */}
                        </span>
                        <span className='text-3xl px-5 pt-5 cursor-pointer md:hidden block'>
                            {
                            show?<FiMenu onClick={() => setShow(false)} />:<AiFillCloseSquare onClick={() => setShow(true)} />
                            }
                        </span>
                    </div>
                    
                    <ul className='md:flex md:flex-row md:items-center z-[-1] md:z-auto md:static 
                    bg-neutral-900 w-full left-0 md:w-auto md:opacity-100 opacity-0 top-[-400px] transition-all ease-in duration-500'>
                    ...
                    <div className='pb-5 pl-5 md:p-0 md:opacity-100 opacity-0 top-[-400px] transition-all ease-in duration-500'><CgProfile className='cursor-pointer text-blue-200 text-4xl'/></div> {/* Find better icon for profile */}
                </nav>
            </div>
        </header>
    );
